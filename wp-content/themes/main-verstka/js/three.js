...
if (serverSecond.status == 200) {
    ...
    img[9].innerHTML = db.kino.reziser.img_1;
    img[12].innerHTML = db.acter.pushkin.img_1;
    img[13].innerHTML = db.acter.shekspir.img_1;
    img[15].innerHTML = db.moda.provintage.img_1;
    img[16].innerHTML = db.moda.classic.img_1;
    img[18].innerHTML = db.tasting.ustizi.img_1;
    img[19].innerHTML = db.tasting.vino.img_1;
    img[20].innerHTML = secondDb.Vino.img;
    ...
    secondTitle.forEach((elem) => {
        if (elem.textContent == '') {
            elem.parentElement.parentElement.parentElement.style.visibility = 'hidden'
        }
    });
    ...
    secondBtn[10].addEventListener('click', eleven);
    secondBtn[11].addEventListener('click', twelve);
    secondBtn[12].addEventListener('click', thirteen);
    secondBtn[13].addEventListener('click', fourteen);
    secondBtn[14].addEventListener('click', fiveteen);
    secondBtn[15].addEventListener('click', sixteen);
    secondBtn[16].addEventListener('click', seventeen);
    secondBtn[17].addEventListener('click', eighteen);
    secondBtn[18].addEventListener('click', nineteen);
    secondBtn[19].addEventListener('click', twenty);
    secondBtn[20].addEventListener('click', twentyOne);
}